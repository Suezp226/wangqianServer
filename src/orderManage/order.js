...
router.get('/exportExcel', function(req, res, next) {
    ...
    consumer.find(qq, {}, (err, docs) => { //定义查询结果显示字段
        ...
        if (!err) {
            ...
            let filename = excel.createReportFile('/order' + date,'sheet1',[],header,list);
        ...
        if (docs.length === 0 || query.phone.trim() == '') {
            ...
            consumer.create([newOrder], (err) => {
                if (!err) {
                    ...
                    // 设计业务员、订货人(普通用户)  
                    // 参数 userType,name,phone,id,company

                    let {ywyName,ywyPhone,bookName,bookPhone,bookIdNum} = newOrder;
